...
- **AI Brain (Agents)** — a LangGraph hybrid-supervisor graph ("Minerva") routes each query to a fast workflow or an agentic specialist loop, then streams a grounded answer.
...
graph TD
    ...
    API --> AG["AI Agents<br/>(LangGraph 'Minerva')"]
...
### Views (15)
Dashboard · Minerva (streaming chat) · Discovery (thematic) · Company workspace · Compare · Portfolio · Watchlist · Domino Effect (causal) · Filings · News · Simulator (paper trading) · Profile · Settings · Auth · Onboarding.
...
"Minerva" is a **single LangGraph `StateGraph`** implementing a **hybrid supervisor**: a router picks between a deterministic *fast workflow* and an *agentic specialist loop*, and both converge on a shared streaming `synthesize` node. Compiled with the **Postgres checkpointer** so sessions persist across turns.
...
### A. A chat query (Minerva)
```mermaid
sequenceDiagram
    participant FE as React (MinervaView)
...
**Working end-to-end:** company universe (~4,900 auto-synced), quotes/financials/news/commodities/geopolitical feeds, portfolios + holdings + risk metrics, paper-trading simulator, the Minerva streaming agent, and the seeded causal graph with market-correlation verification. Auth (OTP + password + demo) with per-user isolation.
