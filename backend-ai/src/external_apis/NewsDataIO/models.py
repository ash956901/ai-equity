...
Free/basic plans return placeholder strings like
"ONLY AVAILABLE IN PROFESSIONAL AND CORPORATE PLANS" for paid-only
fields.  We use ``field_validator`` to coerce those strings to ``None``
so Pydantic doesn't reject the response.
"""
from pydantic import BaseModel, Field, field_validator
from typing import Optional, List, Any, Dict


def _coerce_plan_placeholder(v: Any) -> Any:
    """Return None if the API sent a plan-upgrade placeholder string."""
    if isinstance(v, str):
        return None
    return v
...
class NewsDataArticle(BaseModel):
    ...
    # Pro/Corporate plan fields — may be placeholder strings on free plans
    ...
    @field_validator(
        "ai_tag", "ai_region", "ai_org", "sentiment_stats",
        "keywords", "creator", "country", "category", "symbol",
        mode="before",
    )
    @classmethod
    def _strip_plan_placeholders(cls, v: Any) -> Any:
        return _coerce_plan_placeholder(v)
...
class SentimentFeedArticle(BaseModel):
    ...
    @field_validator("sentiment_stats", mode="before")
    @classmethod
    def _strip_plan_placeholders(cls, v: Any) -> Any:
        return _coerce_plan_placeholder(v)
