...
export function CompareView() {
  ...
  // Run the Minerva verdict whenever both companies are chosen (or changed).
  ...
  useEffect(() => {
    ...
    compareCompanies({
      ...
      .catch(() => {
        if (!cancelled)
          setVerdictError(
            "Minerva could not synthesize a verdict. The metric comparison above is still valid."
          );
      })
      .finally(() => {
      ...
      {!both ? (
        <Card className="text-center py-2xl px-lg">
          ...
          <p className="text-body-sm text-on-surface-variant">
            Select two companies to compare their fundamentals and get a Minerva verdict.
          </p>
        </Card>
      ) : (
        <>
    ...
    <Card className="p-lg">
      <div className="flex items-center justify-between mb-md">
        <div className="flex items-center gap-sm">
          ...
          <span className="text-card-title font-semibold text-on-surface">Minerva Verdict</span>
        </div>
        {loading && (
      ...
      {loading ? (
        <div className="space-y-sm">
          ...
          <p className="text-caption text-on-surface-variant pt-xs">
            Minerva is analysing both entities — this can take up to a minute.
          </p>
        </div>
      ) : error ? (
