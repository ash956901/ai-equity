...
A focused, plain-language design document for the two "brains" of EquityAI:

- **Part 1 — Agentic Design:** how *Minerva*, the AI research analyst, answers questions.
...
# Part 1 — Agentic Design ("Minerva")

## 1.1 The idea in one sentence

> Minerva is a research analyst built as a **graph of steps**. For every question it first decides *"is this a quick lookup or a hard question?"* and then takes one of two routes — a **fast assembly line** or a **thinking loop** — before writing one grounded, streamed answer.
...
So Minerva routes each query to the cheapest strategy that can answer it.

## 1.2 The whole graph

Minerva is a single **LangGraph `StateGraph`** (`src/agents/graph.py`). Five nodes, two paths:
...
Causal Intelligence isn't separate from the agent — it's one of Minerva's specialists:

```mermaid
flowchart LR
    Q["'What if crude spikes?'"] --> MIN["Minerva agent loop"]
